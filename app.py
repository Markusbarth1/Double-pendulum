...
def build_acceleration_functions():
    ...
    acceleration_inputs = (
        theta1, theta2, theta1_dot, theta2_dot, m1, m2, l1, l2, g
    )

    theta1_ddot_func = sp.lambdify(
        acceleration_inputs, theta1_ddot_explicit, "numpy"
    )
    theta2_ddot_func = sp.lambdify(
        acceleration_inputs, theta2_ddot_explicit, "numpy"
    )
...
def rk4_step(state, dt, m1_val, m2_val, l1_val, l2_val, g_val):
    ...
    k2 = derivatives(
        state + 0.5 * dt * k1, m1_val, m2_val, l1_val, l2_val, g_val
    )
    k3 = derivatives(
        state + 0.5 * dt * k2, m1_val, m2_val, l1_val, l2_val, g_val
    )
    k4 = derivatives(state + dt * k3, m1_val, m2_val, l1_val, l2_val, g_val)
...
def simulate(
    ...
    for i in range(1, len(time)):
        states[i] = rk4_step(
            states[i - 1], dt, m1_val, m2_val, l1_val, l2_val, g_val
        )

    return time, states


def calculate_results(states, m1_val, m2_val, l1_val, l2_val, g_val):
...
def make_animation_html(
    ...
    def update(frame):
        ...
        trace_line.set_data(x2_values[: frame + 1], y2_values[: frame + 1])
    ...
    fps = max(1, int(round(1000 / animation_interval)))
...
with st.form("simulation_inputs"):
    m1_val = st.number_input("Mass 1 [kg]", min_value=0.001, value=50.0)
    m2_val = st.number_input("Mass 2 [kg]", min_value=0.001, value=25.0)
    l1_val = st.number_input("Length 1 [m]", min_value=0.001, value=1.0)
    l2_val = st.number_input("Length 2 [m]", min_value=0.001, value=1.0)
    g_val = st.number_input("Gravity [m/s²]", min_value=0.001, value=9.81)

    theta1_deg_val = st.number_input("Initial angle 1 [degrees]", value=120.0)
    theta2_deg_val = st.number_input("Initial angle 2 [degrees]", value=30.0)
    theta1_dot_0 = st.number_input("Initial angular velocity 1 [rad/s]", value=0.0)
    theta2_dot_0 = st.number_input("Initial angular velocity 2 [rad/s]", value=0.0)

    dt = st.number_input("Timestep [s]", min_value=0.0001, value=0.01, format="%.4f")
    t_end = st.number_input("Simulation time [s]", min_value=0.1, value=10.0)
    playback_speed = st.number_input("Playback speed", min_value=0.1, value=1.0)

    start = st.form_submit_button("Start")

stop = st.button("Stop / clear")
...
if start:
    ...
    results = calculate_results(
        states, m1_val, m2_val, l1_val, l2_val, g_val
    )
...
if st.session_state.get("show_simulation", False):
    ...
    st.write(
        ...
        f"{np.max(np.abs(energy_drift)):.6e} J"
    )

    fig_energy, ax_energy = plt.subplots()
    ax_energy.plot(time, energy_drift)
    ax_energy.set_xlabel("Time [s]")
    ax_energy.set_ylabel("Energy drift [J]")
    ax_energy.grid(True)
